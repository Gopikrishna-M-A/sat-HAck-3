...
function reduceOpacity(rgb, opacity): RGBColor {
  ...
  const reducedOpacityColor: RGBColor = {
    r: Math.round(r * 255),
    g: Math.round(g * 255),
    b: Math.round(b * 255),
  };
...
async function createBtn(msg,width,height,type,offsety,offsetx) {
...
async function createNotification(type,message,offsety,offsetx) {
  // Step 1: Load the required font
  await figma.loadFontAsync({ family: "Inter", style: "Regular" });
...
async function createToggle(msg,offsety,offsetx,flag) {
  // Step 1: Load the required font
  await figma.loadFontAsync({ family: "Inter", style: "Regular" });
...
async function createLandingPage(msg) {
  // Step 1: Load the required font
  await figma.loadFontAsync({ family: "Inter", style: "Regular" });
...
figma.ui.onmessage = msg => {
  ...
  if (msg.type === 'create') {
    ...
    async function createFrameWithTextAndClosePlugin() {

      await createBtn(msg,84,36,"pri",0,20);
      await createBtn(msg,84,36,"pri",50,20);
      await createBtn(msg,84,36,"pri",100,20);
      await createBtn(msg,84,36,"pri",150,20);

      await createBtn(msg,84,36,"sec",0,120);
      await createBtn(msg,84,36,"sec",50,120);
      await createBtn(msg,84,36,"sec",100,120);
      await createBtn(msg,84,36,"sec",150,120);

      await createBtn(msg,99,33,"pri",0,220);
      await createBtn(msg,99,33,"pri",50,220);
      await createBtn(msg,99,33,"pri",100,220);
      await createBtn(msg,99,33,"pri",150,220);

      await createBtn(msg,99,33,"sec",0,330);
      await createBtn(msg,99,33,"sec",50,330);
      await createBtn(msg,99,33,"sec",100,330);
      await createBtn(msg,99,33,"sec",150,330);


      await createNotification("success","Congraluations! You've done Something great",50, 0 );
      await createNotification("Beware","You should be careful about this",100, 0 );
      await createNotification("Warning","it may go wrong",150, 0 );
      await createNotification("Reminder","Just a friendly reminder",200, 0);
     
      await createToggle(msg,50,0,true)
      await createToggle(msg,100,0,true)
    
      await createToggle(msg,150,0,false)
      await createToggle(msg,200,0,false)


      await createLandingPage(msg)
